...
M1C <- ggplot() +
  ...
  labs(x="Thermal Tolerance Profile Dates (2023)", y=expression("Temperature "(degree*C)~" ")) +
...
#ED30 model values calculated using PAM data and heat logger data!
...
#ED50
out50<-data.frame(genet=character(),cumulative=numeric(),predicted=numeric(),pmin=numeric(),pmax=numeric())
ED50<-data.frame(genet=character(),ed50=numeric(),se=numeric(),lwr=numeric(),upper=numeric())

for(i in list$genet){
  temp<-resistrecover_pam%>%filter(genet==paste0(i))
  model <- drc::drm(relative_pam~cumulative, data=temp, fct=drc::W1.3(fixed=c(NA,1,NA),names=c("Slope","Upper Limit","ED50")))
  summary(model)  
  summary <- drc::ED(model,c(50), interval="delta")
  ED50<-ED50%>%add_row(genet=i,ed50=summary[1],se=summary[2],lwr=summary[3],upper=summary[4])
  newdata <- expand.grid(cumulative=seq(min(resistrecover_pam$cumulative),max(8),length=100))
  pm <- predict(model,newdata=newdata,interval="confidence")
  newdata<-bind_cols(newdata,pm)
  out50<-out50%>%add_row(genet=i,cumulative=newdata$cumulative,predicted=newdata$Prediction,pmin=newdata$Lower,pmax=newdata$Upper)
  
}
write.csv(ED50,"NIAROX_ED50.csv")

combinedED <- ED %>%
  left_join(.,ED50, by ="genet") %>%
  select(genet, ed30, ed50) %>%
  arrange(ed30) %>%
  mutate(ed30_rank = row_number()) %>%
  arrange(ed50) %>%
  mutate(ed50_rank = row_number()) %>%
  select(genet, ed30, ed30_rank, ed50, ed50_rank) %>%
  arrange(ed30)

write.csv(combinedED, "effective dose ED30 vs ED50.csv")   
...
M3B <- ggplot(rec_pam_data, aes(x=new_time,y=relative_pam,group=coral_id,color=rec_died)) +
  geom_hline(yintercept = 0.7, linetype="solid", color="black",linewidth=1.25) +
  geom_hline(yintercept = 0.6, linetype="dotted", color="black",linewidth=1.25) +
  geom_point(aes(color=rec_died), alpha=0.8)+
  geom_line(aes(color=rec_died, linetype=rec_died)) +
  scale_color_manual(values = c("seagreen4","orange4")) +
  scale_linetype_manual(values = c("Alive" = "solid", "Dead" = "dotted")) +
  annotate("text", x=c(1),y=c(1.3), label=c("131/144 Survivors\nEnded Stress ≥ 0.6 Fv/Fm"),size=2.8) +
  ...
  theme_bw() + ylim(.15,1.4) + theme(axis.title = element_text(size=10),
                     axis.title.x=element_blank(),legend.position="none")

sum(rec_pam_data$rec_died == "Alive" & 
      rec_pam_data$relative_pam >= 0.6)

#131 out of 144 corals between 0.6-0.7 
...
M4A <- ggplot(resistrecover_ED_sa,aes(x=ed30,y=percentchange_avgsa2)) +
  ...
  geom_line(alpha=0.3) +
  ylim(-0.2,1.35) +
  labs(x = "Thermal Tolerance (ED30)", y = expression(log*"%"*Delta~Surface~Area)) + 
  annotate("text", x=c(5.1),y=c(-0.15), label=c("linear p=0.123\nquad p=0.046\n ")) +
...
summary(bw_lm)
bw_quad <- lm(percentchange_avgbw2 ~ ed30 + I(ed30_2), data=resistrecover_ED_bw)
summary(bw_quad)
...
M4C <- ggplot(resistrecover_ED_bw,aes(x=ed30,y=percentchange_avgbw2)) +
  ...
  geom_line(alpha=0.3) +
  ylim(2,3.75) +
  labs(x = "Thermal Tolerance (ED30)", y = expression(log*"%"*Delta~"Buoyant Weight")) + 
  annotate("text", x=c(5.35),y=c(2.05), label=c("linear p=0.93\nquad p=0.09\nd")) +
...
quartz(w=4,h=4)
plot_grid(M4A,M4B,M4C,M4D, ncol=2, labels = c("A","C","B","D"), align="hv", axis="tb", label_size=12, rel_heights = c(1,1),label_y=(1.02))
...
A1_growth_c4_avg <- A1_growth_c4 %>%
  ...
  select(time,temp) %>%
  mutate(time=as.Date(time)) %>%
  add_column(group= c("Growth/Recovery Period")) %>%
  distinct()
...
S1_dates <- seq(from = as.Date("2022-08-25"), to = as.Date("2023-04-27"), by = "day")
#S1A: temperature profiles during growth and heat stress recovery experiments
#S1A_dates <- as.Date(c(A1_growth_c4_avg$time,A1_c4$day,A1_rec_c8_avg$time))
S1A_time <- data.frame(S1A_time=c(A1_growth_c4_avg$time,A2_growth_c6_avg$time,A3_growth_c10_avg$time,A1_c4$day,A2_c5$day,A3_c14$day,A4_c15$day,A1_rec_c8_avg$time,A2_rec_c21_avg$time))
S1A_temp <- data.frame(S1A_temp=c(A1_growth_c4_avg$temp,A2_growth_c6_avg$temp,A3_growth_c10_avg$temp,A1_c4$temp,A2_c5$temp,A3_c14$temp,A4_c15$temp,A1_rec_c8_avg$temp,A2_rec_c21_avg$temp))
S1A_group <- data.frame(S1A_group=c(A1_growth_c4_avg$group,A2_growth_c6_avg$group,A3_growth_c10_avg$group,A1_c4$group,A2_c5$group,A3_c14$group,A4_c15$group,A1_rec_c8_avg$group,A2_rec_c21_avg$group))
...
S1A <- ggplot(S1A_dataset, aes(x=S1A_time,y=S1A_temp,color=S1A_group)) +
  ...
  #geom_line() +
  scale_x_date(
    limits = as.Date(c("2022-08-25", "2023-04-27")),
    date_labels = "%b\n%Y",
    date_breaks = "1 month" 
  ) +
  labs(x=expression("Growth and Heat Recovey Timeline"), y=expression("Temperature "(degree*C))) +
  annotate("rect",
           xmin = as.Date("2023-01-22"),
           xmax = as.Date("2023-02-06"),
           ymin = 22.75,
           ymax = 32.5,
          color = "black", alpha=0) +
...
#S1B: light logger profiles during growth period and heat stress experiment period
...
S1B_dataset <- data.frame(S1B_datetime,S1B_day,S1B_calibrated,S1B_group)


S1B <- ggplot(S1B_dataset, aes(x=S1B_day,y=S1B_calibrated)) +
  geom_point(alpha=0.5, color = "#F8766D") +
  labs(x="", y = expression(atop("PAR Light", (mu * "mol" * "·" * m^{-2} * "·" * s^{-1})))) +
  geom_vline(xintercept = as.Date("2023-01-24"), linetype = "dotted", color="black") +
  geom_vline(xintercept = as.Date("2023-02-06"), linetype = "dotted", color="black") +
  scale_x_date(
    limits = as.Date(c("2022-08-25", "2023-04-27")),
    date_labels = "%b\n%Y",
    date_breaks = "1 month" 
...
quartz(w=7.2,h=4.2)
plot_grid(S1A,S1B, labels=c("A","B"), rel_heights = c(1,1), label_size=12,nrow=2, axis="tblr", align="v")
...
resistrecover_S2B <- resistrecover_ED %>%
  select(genet,percentchange_sa) %>%
  distinct()
resistrecover_S2B$percentchange_sa2 <- log(resistrecover_ED$percentchange_sa)
resistrecover_S2B <- resistrecover_S2B %>%
  na.omit()

S2B <- ggplot(resistrecover_S2B, aes(x=reorder(genet,percentchange_sa2),y=percentchange_sa2)) +
  geom_line() +
  stat_summary(geom="point",fun="mean",size=2) +
  labs(x = "Genet", y=expression(log*"%"*Delta~Surface~Area)) + 
  scale_x_discrete(guide = guide_axis(n.dodge = 2)) +
  annotate("text", x=c(50),y=c(-1), label=c("p = 0.069"),size=3) +
  theme_bw() + theme(axis.text.x=element_text(angle=90,vjust=1,hjust=1),legend.position="none") 

S2B <- ggplot(resistrecover_ED_sa, aes(x=reorder(genet,percentchange_avgsa2))) +
  geom_point(aes(y=percentchange_avgsa2) ,size=2, color = "black") +
  geom_point(aes(y=percentchange_avgsa) ,size=2, color = "red") +
  labs(x = "Genet") + 
  scale_y_continuous(
    name = expression(log*"%"*Delta~Surface~Area),
    sec.axis = sec_axis(trans ~ exp(.), name = expression("%"*Delta~Surface~Area))
  ) +
  scale_x_discrete(guide = guide_axis(n.dodge = 2)) +
  annotate("text", x=c(50),y=c(-1), label=c("p = 0.069"),size=3) +
  theme_bw() + theme(axis.text.x=element_text(angle=90,vjust=1,hjust=1),legend.position="none") 

scale_factor <- max(resistrecover_ED_sa$percentchange_avgsa2, na.rm=TRUE) / 
  max(resistrecover_ED_sa$percentchange_avgsa, na.rm=TRUE)

S2B <- ggplot(resistrecover_ED_sa, aes(x=reorder(genet,percentchange_avgsa2))) +
  # log-transformed points (left y-axis)
  geom_point(aes(y = percentchange_avgsa2, shape = "Log Avg. SA"), color = "black") +
  # raw points scaled to fit left axis
  geom_point(aes(y = percentchange_avgsa * scale_factor, shape = "Avg. SA"), color = "blue") +
  labs(x = "Genet", shape = "Surface Area") +
  scale_y_continuous(
    name = expression(log*"%"*Delta~Surface~Area),
    sec.axis = sec_axis(~ . / scale_factor,
                        name = expression("%"*Delta~Surface~Area))
  ) +
  scale_x_discrete(guide = guide_axis(n.dodge = 2)) +
  annotate("text", x=c(50),y=c(0), label=c("p = 0.069"),size=3) +
  scale_shape_manual(values = c("Log Avg. SA" = 16, "Avg. SA" = 17)) +
...
adult_sa_aov <- aov(percentchange_sa~genet, data=resistrecover_ED)
...
resistrecover_S2D <- resistrecover_ED %>%
  select(genet,percentchange_bw) %>%
  distinct()
resistrecover_S2D$percentchange_bw2 <- log(resistrecover_ED$percentchange_bw)
resistrecover_S2D <- resistrecover_S2D %>%
  na.omit()

S2D <- ggplot(resistrecover_S2D, aes(x=reorder(genet,percentchange_bw2),y=percentchange_bw2)) +
  geom_line() +
  stat_summary(geom="point",fun="mean",size=2) +
...
S2D <- ggplot() +
  geom_line(data=resistrecover_S2D, aes(x=reorder(genet,percentchange_bw2),y=percentchange_bw2)) +
  geom_point(data=resistrecover_ED_bw, aes(x=reorder(genet,percentchange_avgbw2),y=percentchange_avgbw2) ,size=2) +
  labs(x = "Genet", y=expression(log*"%"*Delta~Buoyant~Weight)) + 
  scale_x_discrete(guide = guide_axis(n.dodge = 2)) +
  annotate("text", x=c(50),y=c(2), label=c("p < 0.001"),size=3) +
  theme_bw() + theme(axis.text.x=element_text(angle=90,vjust=1,hjust=1),legend.position="none") 

scale_factor2 <- max(resistrecover_ED_bw$percentchange_avgbw2, na.rm=TRUE) / 
  max(resistrecover_ED_bw$percentchange_avgbw, na.rm=TRUE)

S2D <- ggplot(resistrecover_ED_bw, aes(x=reorder(genet,percentchange_avgbw2))) +
  # log-transformed points (left y-axis)
  geom_point(aes(y = percentchange_avgbw2, shape = "Log Avg. BW"), color = "black") +
  # raw points scaled to fit left axis
  geom_point(aes(y = percentchange_avgbw * scale_factor, shape = "Avg. BW"), color = "blue") +
  labs(x = "Genet", shape = "Buoyant Weight") +
  scale_y_continuous(
    name = expression(log*"%"*Delta~Buoyant~Weight),
    sec.axis = sec_axis(~ . / scale_factor,
                        name = expression("%"*Delta~Buoyant~Weight))
  ) +
  scale_x_discrete(guide = guide_axis(n.dodge = 2)) +
  annotate("text", x=c(50),y=c(1), label=c("p < 0.001"),size=3) +  
  scale_shape_manual(values = c("Log Avg. BW" = 16, "Avg. BW" = 17)) +
  theme_bw() + theme(axis.text.x=element_text(angle=90,vjust=1,hjust=1),
                     axis.title.y = element_text(color = "black"),
                     axis.text.y  = element_text(color = "black"),
                     axis.title.y.right = element_text(color = "blue"),
...
adult_bw_aov <- aov(percentchange_avgbw~genet, data=resistrecover_ED)
summary(adult_bw_aov)

quartz(w=8,h=6)
FigS2_AB <- plot_grid(S2A,S2B, labels=c("A","B"), rel_widths = c(1,1.5), ncol=2)
FigS2_CD <- plot_grid(S2C,S2D, labels=c("C","D"), rel_widths = c(1,1.5), ncol=2)
...
S3 <- ggplot() +
  ...
  geom_hline(yintercept=.7, linetype="dotted") +
  geom_point(data=resistrecover_pam, aes(x=cumulative,y=relative_pam), size=0.5, color="red") +
  facet_wrap(~genet) +
